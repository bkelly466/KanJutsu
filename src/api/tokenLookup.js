...
/**
 * The cache itself — a promise map plus a synchronously-readable map of what
 * lookups have already settled on. Both live in `lookupCache.js`, which is also
 * what the kanji explorer uses; see that file for why a promise cache alone
 * isn't enough to stop the "Looking up…" flash.
 *
 * Module scope, so it survives the overlay unmounting: closing and re-opening
 * the same Token, or tapping it again in a later Sentence, is free.
 */
...
/**
 * The lemma as the cache keys it. Trimmed, and empty means "nothing to search":
 * both functions below short-circuit on it rather than making a request.
 */
 ...
 * A failed lookup is deliberately NOT cached (see `lookupCache.js`) — otherwise
 * a single blip would make that word permanently un-lookupable for the rest of
 * the session, and the retry button in the overlay would do nothing.
 *
